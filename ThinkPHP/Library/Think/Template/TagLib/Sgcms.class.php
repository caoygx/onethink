 ...
 * Html标签库驱动
 */
class Sgcms extends TagLib{
	// 标签定义
	protected $tags   =  array(
			// 标签定义： attr 属性列表 close 是否闭合（0 或者1 默认1） alias 标签别名 level 嵌套层次
			'getData'    => array('attr'=>'model,name,id,limit,sql,where,offset,length','close'=>1),
			'getItem'    => array('attr'=>'model,name,where,sql','close'=>0),
...
//	    var_dump($attr,$content);
		...
		$cacheIterateId = md5 ( implode('', $attr) . $content );
